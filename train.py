	...
	with tf.Session() as sess:
		...
		for i in range(arg_config.epoch_num):
			...
			for j in range(0, training_data_mgr.total_batch, arg_config.batch_size):
				...
				_, loss, acc = sess.run([train_model.train_op, train_model.loss, train_model.accuracy], feed_dict={train_model.x1_input: text1, train_model.x2_input: text2, train_model.label: label})
