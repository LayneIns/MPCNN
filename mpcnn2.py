...
class MPCNN(object):
	...
	def __init__(self, inp1_sequence_length, inp2_sequence_length, num_classes, vocab_size,
		...
		for i, filter_size in enumerate(filter_sizes):
			...
			with tf.variable_scope("BlockB-%s" % filter_size):
				...
				for embedding_idx in range(embedding_size):
	...
	def Algo1 (self,inp1_sentences,inp2_sentences):
		with tf.name_scope("Algo1"):
			...
			feah = []
			for p in range(len(inp1_sentences)):  # for each pooling type p:
				...
				cd = tf.squeeze(cosine_distance(inp1_sentence_normalised, inp2_sentence_normalised, dim=-1), axis=[-1])	#MB X  Num_Filters_for_each_size
				l2diff = tf.sqrt(tf.reduce_sum(tf.square(tf.clip_by_value(tf.subtract(inp1_sentences_stacked,inp2_sentences_stacked,name= "subtract_inp1_inp2"),1e-7,1e+10)),axis=-1))	##MB X  Num_Filters_for_each_size
